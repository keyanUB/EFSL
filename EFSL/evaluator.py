...
class LossEvaluator(BinaryClassificationEvaluator):
    ...
    def __call__(self, model, output_path: str = None, epoch: int = -1, steps: int = -1) -> float:
        ...
        with torch.no_grad():
            for _ in trange(num_batches, desc="Iteration", smoothing=0.05):
                ...
                for b in range(len(sentence_features)):
                  # print(b)
                  sentence_features[b]['input_ids'] = sentence_features[b]['input_ids'].to(device)
                  sentence_features[b]['attention_mask'] = sentence_features[b]['attention_mask'].to(device)
